...
public class MateriaController {
    ...
    public ResponseEntity<?> buscarID(@PathVariable("id") Integer id) {
        Optional<Materia> current = service.findById(id);
    ...
    // To create a record
    @PostMapping("/crear")
    public ResponseEntity<?> crear(@RequestBody Materia entity) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.save(entity));
    }

    // To find one record and update it, specifically by a unique identifier (PK or ID)
    @PostMapping("/editar/{id}")
    public ResponseEntity<?> editar(@PathVariable("id") Integer id, @RequestBody Materia entity) {
        Optional<Materia> optional = service.findById(id);
        if(optional.isPresent()) {
            Materia current = optional.get();
            current.setIdMateria(entity.getIdMateria());
